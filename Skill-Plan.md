...
| `uni:coding-standards` | Gemeinsamer Maßstab (Naming, KISS/DRY, kleine Dateien, Error-Handling): Devs schreiben, Reviewer prüfen danach | CR | WP3 / WP6 |
| `uni:git-workflow` | Feature-Branch → Commit-Konvention → PR; **kein direkter `main`-Push** | CR/WG | WP4–WP6 |
| `uni:codebase-onboarding` | Heterogenes Team versteht Repo-Struktur (Backend-Konzept §7) schnell | VO | WP1 |
...
| Skill | Usecase (projektbezogen) | Schwerpunkt | WP / Auslöser |
|---|---|---|---|
| `uni:feature-dev` | Geführter Einstieg in eine Task (P2/P3-Endpoints) mit Codebase-Verständnis | OP/VO | WP1 |
| `uni:plan` | Vor kritischen/großen Tasks (P1 Contract, P2.4 Logik): Anforderung → Schritte, Risiken | WG/OP | WP2 |
| `uni:api-design` | REST-Naht (Resource-Naming, Status-Codes, Envelope, Versionierung) — G2-Kernverantwortung (P1) | OP | WP2/3 *(DRI: Architekt)* |
| `uni:fastapi-patterns` | Endpoints, Dependency-Injection, Pydantic-Schemas korrekt bauen | OP | WP3 |
| `uni:python-patterns` | Idiomatischer, wartbarer Python-Code | OP | WP3 |
| `uni:error-handling` | Explizites Error-Handling + **Fail-safe** (Ausfall/Stale → nie GRÜN, NF-01) | OP | WP3 |
| `uni:tdd-workflow` | Tests-first für Bewertungslogik (DoD ≥ 80 %, beide Vorfälle) | OP | WP3 |
| `uni:python-testing` | pytest-Unit-/Integrationstests | OP | WP3 |
| `uni:build-fix` | Roten Build/Type-Fehler schnell grün bekommen | WG | WP4 (getriggert) |
| `uni:quality-gate` | Formatter-/Lint-Gate pro Datei | CR/WG | WP4 vor Commit |
| `uni:test-coverage` | Coverage-Lücken vor PR schließen (DoD-Nachweis) | SR/WG | WP5 |
| `uni:pr` | Feature-PR aus Branch erstellen (Push erst nach Genehmigung) | WG | WP5 |
| `uni:checkpoint` | Zwischenstand nach Verifikation sichern | WG | WP4/5 |
...
| Skill | Usecase (projektbezogen) | Schwerpunkt | WP / Auslöser |
|---|---|---|---|
| `uni:code-review` *(geteilt, hier OP)* | Lokaler Diff oder GitHub-PR reviewen — **Hauptwerkzeug** | OP | WP6 bei jedem PR |
| `uni:review-pr` | Umfassendes PR-Review mit spezialisierten Agents | OP | WP6 |
| `uni:verification-loop` | **Pflicht-Gate kritischer Pfad:** Bewertungslogik gegen `Schwellenwerte.md` (beide Vorfälle + Fail-safe als benannte Tests) | OP/WG | WP6/7 (kritisch) |
| `uni:python-review` / `uni:fastapi-review` *(geteilt, OP)* | Fachliche Detail-Reviews | OP | WP6 |
| `uni:security-review` / `uni:security-scan` *(geteilt, OP)* | Ingest, Audit, **RB-01**, Secrets | OP | WP6 |
| `uni:test-coverage` | Coverage prüfen + fehlende Tests generieren (Suite-Pflege) | OP | WP6/7 |
| `uni:python-testing` | Unit-Tests schreiben/pflegen (Agent erstellt, Mensch versteht) | OP | WP6/7 |
| `uni:e2e-testing` | **API-E2E** mit `TestClient`/httpx (Ingest → Bewertung → API, P3.6); Browser-E2E (Playwright) nur bei echter G3-UI-Integration | OP | WP7 (optional) |
...
| `uni:code-tour` | Geführte Tour durch den geänderten Code — **verstehen vor Freigabe** | VO | WP6 |
...
| 2 | Projekthintergrund (Bewertung) | **Entscheidungslogbuch** ist Pflichtdokument + benotet, hatte keinen Skill | Ursprünglich `uni:architecture-decision-records` ergänzt — *v1.3.0 (2026-06-21): Skill entfernt, Logbuch-Pflege jetzt über `uni:entscheidungslog` + `uni:save-session`* |
| 3 | Projekthintergrund (Bewertung) | **API-Doku** ist Pflichtdokument, nicht abgedeckt | Ursprünglich `uni:update-docs` ergänzt — *v1.3.0 (2026-06-21): Skill entfernt (Redundanz), API-/Doku-Sync in `uni:save-session` gefaltet* |
...
|---|---|---|
| **P1** | `uni:database-migrations` zielt auf Postgres/MySQL + ORMs → widerspricht SQLite-Ausschluss | Aus dem Set **entfernt**; SQLite-Init per `CREATE TABLE`/Backend-Konzept §7; Migrations-Skill erst T2+ |
| **P1** | Kein echtes **Verifikations-Gate** für die sicherheitskritische Bewertungslogik (nur Coverage) | `uni:verification-loop` als Pflicht-Gate ergänzt; **DoD kritischer Pfad** = beide Vorfälle als benannte, grüne Tests + Fail-safe-Test |
...
| **P2** | ADR ≠ Format des benoteten **Entscheidungslogbuchs** | ADR als **Rohstoff** klargestellt — *v1.3.0 (2026-06-21): `uni:architecture-decision-records`/`uni:update-docs` entfernt; Logbuch über `uni:entscheidungslog` + `uni:save-session`* |
...
---
*Toolkit-Version: v1.4.0 · Stand: 2026-06-21 (urspr. 2026-06-17) · Pflege: Lucas (Systemarchitekt) · Use-Case-Fakten stets aus `Alarmsystem-Dev` lesen.*
