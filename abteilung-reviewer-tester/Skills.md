...
|---|---|---|---|
| `ecc:code-review` *(geteilt, hier **OP**)* | **Hauptwerkzeug:** lokalen Diff oder GitHub-PR strukturiert reviewen; Befunde nach Schweregrad | **OP** | WP6 bei jedem PR |
| `ecc:review-pr` | Umfassendes PR-Review mit mehreren spezialisierten Agents (Mehr-Perspektiven) | **OP** | WP6 |
| `ecc:python-review` / `ecc:fastapi-review` *(geteilt, OP)* | Fachliche Detail-Reviews (Python-Idiome / Async-DI-Pydantic-OpenAPI) | **OP** | WP6 |
...
> `ck` (Start) · `code-tour` (Change verstehen) · `code-review` (reviewen) · `test-coverage` (Tests prüfen) ·
...
1. **WP0** `ck` → Kontext. Neuer PR offen → **WP6**.
